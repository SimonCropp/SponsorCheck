...
This is the central design. Hold it in mind when reading any file in `SponsorCheck.Tasks/`.
...
4. Also packs the multi-targeted `tasks/{netstandard2.0,net472}/SponsorCheck.Tasks.dll`.
...
`ConsumerVerifier.targets` (in `SponsorCheck.Tasks/EmbeddedTemplates/`) uses `__SC_PACKAGE_ID__` and `__SC_PACKAGE_ID_RAW__` as placeholders. The bundler substitutes them at pack time:
...
## Multi-targeting in `SponsorCheck.Tasks`

Targets `netstandard2.0;net472`. Both must build because the consumer's MSBuild may be either Core (.NET) or Framework (Visual Studio / MSBuild.exe). `SponsorCheck.targets` and the generated verifier targets pick the right TFM via `'$(MSBuildRuntimeType)' == 'Core'` conditionals.
