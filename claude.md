...
- `NoWarn`, `MSBuildWarningsAsMessages`, `MSBuildWarningsAsErrors` and `MSBuildTreatWarningsAsErrors` are forwarded into the announce build. MSBuild applies warning suppression from the project that *logs* the warning, so without them a consumer's `<NoWarn>SC005</NoWarn>` would silently stop working. They are part of the cache key, so projects that suppress differently announce separately.
