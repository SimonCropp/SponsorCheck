...
public static class AuthorConfigGenerator
{
    ...
    static string Version(AuthorModel model) =>
        string.IsNullOrWhiteSpace(model.SponsorCheckVersion)
            ? WizardDefaults.SponsorCheckVersion
            : model.SponsorCheckVersion.Trim();
    ...
    {
        var source = !string.IsNullOrWhiteSpace(model.OwnerId)
            ? model.OwnerId
