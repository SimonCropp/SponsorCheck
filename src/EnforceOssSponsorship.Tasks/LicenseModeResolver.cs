...
public abstract record LicenseDecision(string PackageId)
{
    ...
    public sealed record Sponsor(string PackageId, IReadOnlyDictionary<string, string> AccountByPlatform) : LicenseDecision(PackageId);
...
public static class LicenseModeResolver
{
        ...
        string packageId)
    {
        ...
        if (nonEmptySponsors.Count > 0)
        {
            return new LicenseDecision.Sponsor(packageId, nonEmptySponsors);
        }
