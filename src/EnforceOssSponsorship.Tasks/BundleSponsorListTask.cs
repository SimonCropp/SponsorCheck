...
public sealed class BundleSponsorListTask : Microsoft.Build.Utilities.Task
{
    ...
    public string GitHubSponsorsToken { get; set; } = "";
        ...
        try
        {
            ...
            if (enabled.Count == 0)
            {
                Log.LogError(
                    ...
                    0,
                    0,
                    "EnforceOssSponsorship: at least one platform account metadata must be set on the PackageReference or PackageVersion (e.g. GitHubSponsorsAccount=\"acmecorp\").");
                return false;
            }
            ...
            var rendered = template
                .Replace("__EOSS_PACKAGE_ID__", sanitizedId)
                .Replace(">__EOSS_PACKAGE_ID_RAW__<", $">{ThePackageId}<");
            File.WriteAllText(OutputVerifierTargetsPath, rendered);

            Log.LogMessage(
                MessageImportance.High,
                $"EnforceOssSponsorship: bundled {hashes.Count} sponsor entries across {entries.Select(e => e.Platform).Distinct(StringComparer.OrdinalIgnoreCase).Count()} platform(s) into '{ThePackageId}'.");
            return true;
        }
        catch (MaintenanceFeeException ex)
        {
            Log.LogError("EOSS", "EOSS100", "", "", 0, 0, 0, 0, ex.Message);
            return false;
        }
        catch (Exception ex)
        {
    ...
    {
        Log.LogMessage(MessageImportance.High, $"EnforceOssSponsorship: using sponsor override list '{OverrideListPath}'.");
    ...
    IReadOnlyDictionary<string, string> LoadUserSecrets()
    {
        ...
        try
        {
            ...
            if (secrets.Count > 0)
            {
                Log.LogMessage(MessageImportance.Low, $"EnforceOssSponsorship: loaded {secrets.Count} user-secrets from id '{UserSecretsId}'.");
            }
        ...
        {
            Log.LogWarning("EOSS", "EOSS104", "", "", 0, 0, 0, 0,
                $"EnforceOssSponsorship: could not read user-secrets at '{UserSecretsReader.ResolvePath(UserSecretsId)}': {ex.Message}");
            return new Dictionary<string, string>();
        }
    }
    ...
    string? TokenFor(string platformId)
    {
        ...
        {
            "GitHubSponsors" => NullIfEmpty(GitHubSponsorsToken),
        ...
        // Fall back to user-secrets convention: EnforceOssSponsorship:<Platform>Token
        var key = $"EnforceOssSponsorship:{platformId}Token";
