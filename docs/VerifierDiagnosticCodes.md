...
Every emitted message is prefixed with the code's short **Name** (e.g. `No license specified. Package 'MyOssLib'...`) and ends with `See: https://github.com/SimonCropp/SponsorCheck/blob/main/docs/VerifierDiagnosticCodes.md#<code>`. An error or warning carries that link on a line of its own, after a blank line that sets it apart from the remediation block. A message-severity diagnostic whose body is a single line keeps the link on that line, so each audit record (SC017, SC029–SC031, SC059) is one line of build output: the console logger repeats the file, code and project on every line of a multi-line message. The Syntax/Example entries below show the inner format string only — the name and link wrap is added at log time.

Each distinct message is reported **once per build** — not once per project, and not once per target framework of a multi-targeted one. The verifier itself still runs everywhere it did before, so nothing about enforcement changes; only the reporting collapses. Two projects needing *different* fixes render different messages, so each is still reported separately and nothing that names a file to fix is lost. A warning is also reported once per distinct set of warning settings (`NoWarn`, `MSBuildWarningsAsMessages`, `MSBuildWarningsAsErrors`, `MSBuildTreatWarningsAsErrors`), since those decide whether it is suppressed or promoted; they have no effect on a message or an error, so projects whose warning settings differ still share one report of the same audit line.

The message-severity codes (SC017, SC029–SC031, SC059) are logged at **high** importance on a build server and **low** importance elsewhere. The audit trail those codes carry belongs in the CI log, which is what gets read after the fact and which defaults to minimal verbosity; on a developer machine the same line repeats on every build while nothing is being recorded, so it sits below the default log and `-v detailed` brings it back. Errors and warnings are unaffected — importance does not apply to them.

That is the default. A consumer can choose how messages and warnings are logged instead, with the `SponsorCheckMessageLevel` and `SponsorCheckWarningLevel` properties (see [Log levels](ConsumerUsage.md#log-levels)): a message can be logged at any importance or raised to a warning, and a warning can be lowered to a message. Errors are never affected. The **Level** given for each code below is its default.
...
- **Level**: Error
- **Meaning:** `SponsorCheckMessageLevel` or `SponsorCheckWarningLevel` holds something other than `warning`, `high`, `normal` or `low` (see [Log levels](ConsumerUsage.md#log-levels)). Checked before verification, so it fails a build that would otherwise pass without a word — on a quiet build a typo there is never noticed any other way. The properties are global, so every SponsorCheck package in the build reports it and one fix clears them all. Placement-agnostic. Not overrideable.
- **Syntax:** `Package '{PackageId}': {property}='{value}' is not a log level. Valid levels are warning, high, normal and low; an unset property keeps the default.` (one `is not a log level` sentence per invalid property.)
- **Example:** `Package 'Verify': SponsorCheckMessageLevel='quiet' is not a log level. Valid levels are warning, high, normal and low; an unset property keeps the default.`
