...
The charts below are **justification / contrast views**: they map what each scenario produces and what trace it leaves behind. They are a different altitude from the mechanism flowchart in [How verification works](ConsumerUsage.md#how-verification-works), which shows the verifier's internal decision logic. That chart is not repeated here.
...
flowchart TD
    ...
    SC029([<a href='https://github.com/SimonCropp/SponsorCheck/blob/main/docs/VerifierDiagnosticCodes.md#sc029'>Passes, logs an audit message on every build #40;SC029#41;<br/>durable marker quoting publisher's criteria text<br/>not a breach - audit-trail for a sanctioned carve-out</a>])
...
The publisher-defined exemption path also passes with a recurring marker, but unlike `SponsorshipLicenseIgnored` the marker is a high-priority message quoting the publisher's own criteria text rather than a generic "in breach" warning — so a consumer who legitimately qualifies (e.g. a consulting client under a 6-month carve-out, or a sub-$10k-revenue business) is not mislabeled, and a warnings-as-errors gate doesn't turn a sanctioned claim into a failed build. Each exemption name must be declared by the publisher at pack time, so the consumer cannot invent one; an unknown name fails with [SC032](VerifierDiagnosticCodes.md#sc032), and the error body lists what the publisher does offer.
